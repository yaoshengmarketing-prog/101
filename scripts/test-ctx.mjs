...
check("開賽後、有賽前紀錄：checks 原封不動沿用，記下凍結時間與距表定開賽分鐘", (() => { const r = decide(game(L), pre, live, NOW); return r.checks === pre.checks && r.sample === "pregame" && r.frozen.at === NOW && r.frozen.minutesBeforeScheduledStart === 665; })());
...
check("賽後：主隊贏、九下沒打＝ notPlayed（確定沒打），不是 missing", po.notPlayed.length === 1 && po.notPlayed[0].n === 9 && po.notPlayed[0].side === "home" && po.missing.length === 0 && po.complete === true);
// 完賽 ≠ 資料完整
const cut = (f) => { const e = JSON.parse(JSON.stringify(fe)); f(e); return e; };
const g7 = cut(e => { e.linescore.teams.home.runs = 3; e.linescore.teams.away.runs = 2; e.linescore.innings = [1, 2, 3, 4, 5, 6, 7, 8, 9].map(n => ({ num: n, away: { runs: n === 1 ? 2 : 0 }, home: n === 7 ? {} : n === 8 ? { runs: 2 } : n === 9 ? {} : { runs: 0 } })); });
const p7 = summarizePost(g7, box, NOW);
check("GPT 案例：主隊 3 分、七局 1 分（欄位缺）、八局 2 分 → 後段不算成 2，記未取得", p7.late.home === null && p7.missing.some(m => m.n === 7 && m.side === "home"));
check("GPT 案例：完賽但資料未齊 → gameOver、complete＝false、done＝false（下次再補）", p7.gameOver && p7.complete === false && p7.done === false && p7.incomplete.includes("7下得分"));
check("GPT 案例：已取得的部分照存（比分、其他局、客隊後段、投手）", p7.final.home.runs === 3 && p7.innings[7].home === 2 && p7.late.away === 0 && p7.pitching.away.rp.apps === 2);
check("GPT 案例：九下缺值但已知加總 2 ≠ 總分 3 → 不當成沒打", !p7.notPlayed.some(m => m.n === 9));
check("中間整局缺（innings 少一局）→ 兩個半局都記未取得", (() => { const p = summarizePost(cut(e => { e.linescore.innings = e.linescore.innings.filter(i => i.num !== 4); }), box, NOW); return p.missing.filter(m => m.n === 4).length === 2 && !p.complete; })());
check("逐局都有但加總對不上總分 → 不完整", (() => { const p = summarizePost(cut(e => { e.linescore.teams.away.runs = 4; }), box, NOW); return !p.complete && p.incomplete.some(x => x.includes("對不上")); })());
check("box score 沒取得 → 比分逐局照存，但不完整", (() => { const p = summarizePost(fe, null, NOW); return p.final.away.runs === 3 && p.pitching === null && !p.complete && !p.done; })());
check("post v0.1 的舊紀錄（沒分沒打／未取得）重算一次", pending({ game: { startUTC: "2026-09-25T20:00:00Z" }, post: { rules: "post v0.1", done: true } }, Date.parse(NOW)) && !pending({ game: { startUTC: "2026-09-25T20:00:00Z" }, post: { rules: "post v0.2", done: true } }, Date.parse(NOW)));
check("完賽但資料未齊的紀錄會再查", pending({ game: { startUTC: "2026-09-25T20:00:00Z" }, post: { gameOver: true, complete: false, done: false } }, Date.parse(NOW)));

...
| 2026-09-26 02:3x | Claude | 101 workflow（`b2edd79`，網頁上傳） | checkout 加 `ref: main`：排隊的 run 開始時取 main 最新版，不再用觸發當下的舊版（原因見下方「維護紀錄」） |
...
首晚實測的 2–3 分鐘，是「**排定觸發時間 → 網站部署完成**」的實測值，不是「來源資料變動 → 頁面更新」的保證。來源在兩個時段之間變動時，要等下一個 15 分鐘時段才會被抓到；GitHub 本身也可能延遲。

### 維護紀錄

**2026-09-25 01:01（台灣）run `36031388985` commit 失敗**
- 現象：取數、牛棚、QA 都成功；commit 步驟的 `git pull --rebase` 在 `data/`、`live/data/` 多個檔案發生內容衝突（日誌：`d094e40..2106ded main -> origin/main`、`CONFLICT (content)`）。
- 原因：GitHub 原生排程 run `36031364551`（17:01:08Z 建立）和 Cloudflare 觸發的 run（17:01:20Z 建立）幾乎同時。後者依 concurrency 排隊，等前者推上 `2106ded` 才開始，但 `actions/checkout` 預設取「觸發當下」的版本 `d094e40`，拿舊資料當基準產生新資料，和前者的提交衝突。
- 影響：這一輪的資料沒推上去；網站仍是前者 17:01:25Z 推上的版本。依頁面邏輯，「最近一次自動更新執行失敗」紅色提示會顯示到下一個成功 run 結束（約 15 分鐘）。
- 通知與恢復：失敗通知步驟開了 issue #1（17:01:44Z）；下一輪 01:15 run `36033098099` 成功後自動留言並關閉（17:16:25Z）。01:15、01:30、01:45 都成功。
- 範圍：9/24 13:00Z 起 91 個 live-data run，只有這 1 個遇到「排隊期間前一個 run 推了新提交」，也是唯一的失敗（失敗 1/91）。
- 修正（最小）：checkout 加 `ref: main`，排隊的 run 開始時取 main 最新版。沒有改成強制覆蓋，`git pull --rebase` 照舊。
- 驗證：修正後第一個 run `36175484527`（cf 18:45Z）日誌顯示 `ref: main`；之後 18:45–19:45 共 5 個 run 都成功。「兩個 run 撞在同一分鐘」的情況還沒再發生，下次發生時才是實際驗證；若再失敗，會照常開 issue。

**2026-09-25 19:41（台灣）issue #2「超過 6 小時沒有成功更新」**
- 白天不在 Cloudflare 時段（11:45–21:00），只靠 GitHub 原生排程：12:46 跑過之後，下一次是 19:40（原定 14:07、17:07 都沒跑）。同一個 run 開 issue、10 秒後成功並關閉。
- 白天沒有進行中的比賽，影響小；暫不調整。

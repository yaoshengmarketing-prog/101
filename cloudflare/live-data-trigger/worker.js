...
async function resolve(env) {
  const { results } = await env.DB.prepare("SELECT id, run_id FROM triggers WHERE http BETWEEN 200 AND 299 AND conclusion IS NULL AND scheduled_at > ?")
  ...
  for (const t of results) {
    ...
    await env.DB.prepare("UPDATE triggers SET run_id=?, run_status=?, conclusion=?, run_created_at=?, run_started_at=?, run_updated_at=?, checked_at=? WHERE id=?")
      .bind(run.id, run.status, run.conclusion, run.created_at, run.run_started_at, run.updated_at, now, t.id).run();
  }
}
